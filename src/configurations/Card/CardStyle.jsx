
const CardStyles = {
  style1: 'bg-cyan-50 rounded-sm shadow-md p-4 m-7 w-72 size-40 text-teal-600 text-semibold text-xl font-semibold',
  style2: 'bg-purple-50 rounded-sm shadow-md p-4 m-7 w-72 size-40 text-purple-600 text-semibold text-xl font-semibold',
  style3: 'bg-blue-50 rounded-sm shadow-md p-4 m-6 w-72 size-40 text-blue-600 text-semibold text-xl font-semibold',
  homestyle1: 'bg-white-50 rounded-md shadow-md p-4 m-4 w-[50vh] size-40 h-32 border ',
  homestyle2:'flex flex-col rounded-md  p-4 w-29 bg-white shadow-md mx-4 w-[159vh] h-[55vh] border',
  homestyle3: 'bg-white-50 rounded-md shadow-md p-4 m- w-[50vh] size-40 h-[38vh] border',
  CardTitle: 'text-xl font-bold mb-2',
  CardContent: 'mx-4 text-blue-600 text-2xl font-semibold ',
  MarginBetweenCards: 'my-5',
  homebarchart:'w-[70vh] h-[30vh]',
  homepichart2:'w-[25vh] h-[25vh] mt-4 ml-[11vh]  ',
  homeiconstyle1:' -mt-[4vh]  text-red-400',
  homecontent:' text-gray-600 ml-36 mt-8 text-lg font-bold w-6',
  homecontent2:' text-gray-600 text-center mt- text-lg font-bold ',
/////////////////////////////////////////////////
reportstyle1: 'bg-white rounded-sm border-t border-s shadow-md p-4 m-4 w-64 size-40 h-32 ',
reportstyle3: 'bg-white rounded-sm border-t border-s shadow-md p-4 m-4 w-[37vh] size-40 h-32 ',
reportstyle2:'bg-white flex flex-col  rounded-sm border-t border-s px-1 py-4  min-w-72 min-size-40  shadow-md mx-4',
reportbarchart:'w-[77vh] h-[32vh]',
reportpichart:'w-[25vh] h-[25vh] -mt-12 ml-24 ',
reporticonstyle1:'-mt-8   ml-[24vh] ',
reportcontent:"mt-3 ml-4 text-gray-600 text-lg font-bold",


};



export default CardStyles