


// CardStyles.js
const CardStyles = {
  style1: 'bg-cyan-50 rounded-sm shadow-md p-4 m-7 w-72 size-40 text-teal-600 text-semibold text-xs',
  style2: 'bg-purple-50 rounded-sm shadow-md p-4 m-7 w-72 size-40 text-purple-600 text-semibold text-xs',
  style3: 'bg-blue-50 rounded-sm shadow-md p-4 m-6 w-72 size-40 text-blue-600 text-semibold text-xs',
  style4: 'bg-white-50 rounded-sm shadow-md p-4 m-4 w-[50vh] size-40 h-32  ',
  style5:'flex flex-col rounded-sm  p-4 w-29 bg-white shadow-md mx-4 w-[157vh] h-[70vh]',
  style6: 'bg-white-50 rounded-sm shadow-md p-4 m- w-[50vh] size-40 h-[38vh] ',
  CardTitle: 'text-xl font-bold mb-2',
  CardContent: 'mx-4 text-blue-600 text-2xl font-semibold ',
  MarginBetweenCards: 'my-5',
  barchart:'w-[70vh] h-[30vh]',
  pichart:'w-[0vh] h-[20vh] -mt-10 ml-24',
  pichart2:'w-[25vh] h-[25vh] mt-4 ml-[11vh]  ',
  iconstyle1:'-mt- 6 ml-44 ',
  iconstyle2:' -mt-[4vh]  text-red-400',
  homecontent:' text-gray-600 ml-36 mt-8 text-lg font-bold w-6',
  homecontent2:' text-gray-600 text-center mt- text-lg font-bold ',
};



export default CardStyles